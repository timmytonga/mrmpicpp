...
void masterSendPath2(char * path, queue<string> &masterWQ){ // explore path and send workers work
    struct stat filestat;
    int status;
    status = stat(path, &filestat);
    queue<string> sendWQ; // this is a queue to hold data ready to scatter to slaves
    ...
    if (S_ISDIR(filestat.st_mode)){
        ...
        while(1){
            ...
            if (dest == 0) {
                ...
                sendWQ.push(newpath); // push this path on the sendWQ
            }
...
void wordcount(const char * path, map<string,int> & result){
    struct stat filestat;
    int status;
    status = stat(path, &filestat);
...
void slaveMap(map<string, int> &result, queue<string> workqueue){
    /* MULTITHREAD THIS !!!! */
    while (!workqueue.empty()){ // while not empty
        wordcount(workqueue.front().c_str(), result);
